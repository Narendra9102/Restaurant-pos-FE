...
function GenerateBill() {
  ...
  const handleGenerateBill = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:8000/api/restaurant/bills/generate/', {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
      console.log("Generate bill:", data)
  ...
  const handleMarkPaid = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8000/api/restaurant/bills/mark-paid/${billData.bill_id}/`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        }
          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-gray-800">Generate Bill</h1>
          ...
          <div className="bg-white rounded-lg shadow p-6 mb-6 print:hidden">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-600">Status: {tableInfo.status}</p>
              </div>
              {!billData && (
                <button
                  onClick={handleGenerateBill}
                  disabled={loading}
                  className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-between mb-2">
                ...
                <span className="font-semibold text-orange-600">{billData.status}</span>
              </div>
            </div>
